  ...
  (cond 
    ...
    ((list? expr)
      (let ((operator (car expr)) (operands (cdr expr)))
        (cond
          ((equal? operator 'add)
            (+ (eval-expr1 (car operands)) (eval-expr1 (cadr operands))))
          ((equal? operator 'sub)
            (- (eval-expr1 (car operands)) (eval-expr1 (cadr operands))))
          ((equal? operator 'mul)
            (* (eval-expr1 (car operands)) (eval-expr1 (cadr operands))))
          ((equal? operator 'uminus)
            (- (eval-expr1 (car operands)))))))))
  ...
  (cond
    ...
    ((symbol? expr)
     (let ((value (assoc expr env)))
       (if value
           (cadr value)
           (error (symbol->string expr) "undefined"))))
    ((list? expr)
     (let ((operator (car expr)) (operands (cdr expr)))
       (cond
        ((equal? operator 'add)
         (+ (eval-expr2 (car operands) env)
            (eval-expr2 (cadr operands) env)))
        ((equal? operator 'sub)
         (- (eval-expr2 (car operands) env)
            (eval-expr2 (cadr operands) env)))
        ((equal? operator 'mul)
