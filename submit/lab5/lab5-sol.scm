...
(define (sum-lengths-tr ls (sum 0))
  (if (null? ls) sum
         (sum-lengths-tr (cdr ls) (+ (length (car ls)) sum))))
...
(define (poly-eval x coeffs (sum 0) (xPow 1))
  ( if (null? coeffs) sum
       (poly-eval x (cdr coeffs) (+ (* xPow (car coeffs)) sum) (* xPow x))))
...
(define (greater-thans ls (val 0))
  (filter (lambda (list) (> list val)) ls))
